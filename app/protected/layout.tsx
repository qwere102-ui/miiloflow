        ...
        <nav className="w-full flex justify-center border-b border-b-foreground/10">
          <div className="w-full max-w-5xl flex flex-wrap gap-y-2 justify-between items-center p-3 px-5 text-sm">
            <div className="flex flex-wrap gap-3 sm:gap-5 items-center font-semibold">
              <Link href={"/"}>MiiloFlow</Link>
              <Link href={"/protected/instagram"} className="font-normal text-foreground/70 hover:text-foreground">
                인스타그램 계정
              </Link>
              <Link href={"/protected/automations"} className="font-normal text-foreground/70 hover:text-foreground">
                자동화
              </Link>
            </div>
            {!hasEnvVars ? (
